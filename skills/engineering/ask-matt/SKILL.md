...
- **`/whiteboard-defense`**: a lesson on a shipped system so you can defend it at a whiteboard: how it works, why X and not Y, the malicious actor, the data structures, where it fails. Every reason is sourced from the repo or labelled unrecorded or inherited. Stateless and conversational, unlike `/teach`, which is a multi-session course.
